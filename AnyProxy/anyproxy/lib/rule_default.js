...
module.exports = {
    ...
    replaceServerResDataAsync: function(req,res,serverResData,callback){    
         if(/mp\/getmasssendmsg/i.test(req.url)){//当链接地址为公众号历史消息页面时(第一种页面形式)
           console.log("第一种形式 x-wechat-uin :"+req.headers['x-wechat-uin']);
           console.log("第一种形式 x-wechat-key :"+req.headers['x-wechat-key']);
          
           //console.log("params:"+params.uin);
           params = "&uin="+req.headers['x-wechat-uin']+"&key="+req.headers['x-wechat-key'];
           HttpPost(params, req.url, "getWxHis.php");
            if(serverResData.toString() !== ""){
                try {//防止报错退出程序
                    ...
                    var http = require('http');
                    http.get('http://localhost:80/getWxHis.php', function(res) {//这个地址是自己服务器上的一个程序，目的是为了获取到下一个链接地址，将地址放在一个js脚本中，将页面自动跳转到下一页。后文将介绍getWxHis.php的原理。
                        res.on('data', function(chunk){
            ...
            callback(serverResData);//直接返回第二页json内容
                ...
                //console.log("第二种形式 x-wechat-uin :"+req.headers['x-wechat-uin']);
                //console.log("第二种形式 x-wechat-key :"+req.headers['x-wechat-key']);
                 params = "&uin="+req.headers['x-wechat-uin']+"&key="+req.headers['x-wechat-key'];            
                //console.log("params:"+params.uin);
                HttpPost(params, req.url, "getWxHis.php");
                  try {
                    ...
                    // ret[1]=ret[1].replace(/\\/g, '');
                     //ret[1]=ret[1].replace(/amp/g, '');
                     //ret[1]=ret[1].replace(/&:&/g, ':');
                     //ret[1]=ret[1].replace(/&;;/g, '&');
                     console.log("第二种形式ret:"+ret[1]);
                     HttpPost(ret[1],req.url,"getMsgJson.php");//这个函数是后文定义的，将匹配到的历史消息json发送到自己的服务器
                     var http = require('http');
                     http.get('http://localhost:80/getWxHis.php', function(res) {//这个地址是自己服务器上的一个程序，目的是为了获取到下一个链接地址，将地址放在一个js脚本中，将页面自动跳转到下一页。后文将介绍getWxHis.php的原理。
                     console.log("getWxHis statusCode"+res.statusCode);
                       res.on('data', function(chunk){
                        callback(chunk+serverResData);//将返回的代码插入到历史消息页面中，并返回显示出来
                        console.log("callback第二种形式请求成功:"+chunk);
                    })
                });
            }catch(e){
               ...
               try {
                var http = require('http');
                http.get('http://localhost:80/getWxPost.php', function(res) {//这个地址是自己服务器上的另一个程序，目的是为了获取到下一个链接地址，将地址放在一个js脚本中，将页面自动跳转到下一页。后文将介绍getWxPost.php的原理。
                    res.on('data', function(chunk){
