<?php
//getWxHis.php 当前，页面为公众号历史消息时读取这个程序
...
$schem = "http://mp.weixin.qq.com";
$str = $_POST['str'];
$url = $_POST['url'];//先获取到两个POST变量

/*if(){
	if(strpos($url, $schem)===false){
	$content_url="$schem"."$url"."$str";
	}else{
		$content_url="$url"."$str";
	}
}
...
$collection = $db->UGCZone;

$collection->insert(array("content_url"=>$content_url));
